...
export default function Home() {
  ...
  function deleteUserData(id: string | number) {
    setUserData(prev=>prev.filter((_,index)=>index!=id))
  }
        ...
        <div className=" w-full flex gap-2">
          ...
          <Button onClick={handleInputValue} />
            ...
            <div className=" bg-slate-300 rounded-md   min-h-36 max-h-40 p-2 overflow-y-auto">
              {userDatas.map((item,index)=> <div key={`todo-${index}`}> <div className=" flex my-1 justify-between items-center"> 
              <p className=" font-semibold ">{index+1}) {upperCase(item)}</p> <button onClick={()=>deleteUserData(index)} className=" text-slate-100 px-2 rounded-md bg-red-800">X</button>
              </div></div>)}
